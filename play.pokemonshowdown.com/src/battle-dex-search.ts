...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	protected formatType: 'doubles' | 'bdsp' | 'bdspdoubles' | 'rs' | 'frlg' | 'bw1' | 'letsgo' | 'metronome' | 'natdex' |
		...
		'svdlc1natdex' | 'stadium' | 'lc' | 'legendsza' | 'superchampions' | null = null;
	isDoubles = false;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | Dex.PokemonSet = '' as ID) {
		...
		if (format.startsWith('gen')) {
			const gen = (Number(format.charAt(3)) || 9);
		...
		if (format.includes('superchampions')) {
			this.formatType = 'superchampions';
	...
	protected firstLearnsetid(speciesid: ID) {
		...
		if (this.formatType === 'superchampions') table = table['superchampions'];
	...
	getTier(pokemon: Dex.Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'superchampions' ? `superchampions` :
			`gen${gen}`;
		if (table?.[tableKey]) {
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		} else if (this.formatType === 'superchampions') {
			table = table[`superchampions`];
		} else if (isVGCOrBS) {
...
class BattleMoveSearch extends BattleTypedSearch<'move'> {
	...
	getBaseResults() {
		...
		if (this.formatType === 'superchampions') lsetTable = lsetTable['superchampions'];
